...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>
  * { margin: 0; box-sizing: border-box; }
  ...
  .mark { width: 56px; height: 56px; border-radius: 16px; background: #C8FA4B; display: grid; place-items: center; }
  .name { font-size: 34px; font-weight: 900; letter-spacing: -.02em; line-height: 1; }
  .unit { font-size: 13px; font-weight: 800; letter-spacing: .22em; color: #C8FA4B; margin-top: 6px; }
  ...
  h1 em { font-style: normal; color: #C8FA4B; }
  ...
  .badge { font-size: 13px; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: #06070A;
           background: #C8FA4B; padding: 9px 16px; border-radius: 999px; white-space: nowrap; }
</style></head><body>
  <svg class="art" viewBox="0 0 1200 630" preserveAspectRatio="none">
    <defs>
      <radialGradient id="g" cx="80%" cy="14%" r="70%">
        <stop offset="0%" stop-color="#C8FA4B" stop-opacity=".2"/>
        <stop offset="100%" stop-color="#C8FA4B" stop-opacity="0"/>
      </radialGradient>
    </defs>
    <rect width="1200" height="630" fill="#080A0D"/>
    <rect width="1200" height="630" fill="url(#g)"/>
    <g transform="rotate(-24 600 315)">
      ${Array.from({ length: 14 }, (_, i) => `<rect x="${-300 + i * 118}" y="-200" width="${i % 3 === 0 ? 22 : 7}" height="1030" fill="#C8FA4B" opacity="${i % 3 === 0 ? 0.1 : 0.05}"/>`).join('')}
    </g>
    <g fill="none" stroke="#C8FA4B" stroke-width="6" stroke-linecap="round" opacity=".14"
  ...
  <div class="row">
    <div class="mark">
      <svg viewBox="0 0 32 32" width="34" height="34" fill="none">
        <path d="M4 21 L13 7 L17 14 L21 10 L28 21" stroke="#0A0C10" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
        <circle cx="24.5" cy="24.5" r="2.5" fill="#0A0C10"/>
      </svg>
    </div>
